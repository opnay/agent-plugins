...
`code-quality`는 생산 코드 작성, 수정, 리팩터링, 테스트, 의존성 판단, 코드 리뷰를 위한 플러그인입니다.
...
- `$code-quality:code-quality`로 구현, 버그 수정, 리팩터링, 테스트 추가, 의존성 검토, 코드 리뷰를 수행합니다.
...
위 작업은 사용자가 `$code-quality:code-quality`를 명시적으로 호출했거나, correctness/security/compatibility/data-integrity/operational-risk 같은 code-quality 검토를 직접 요청한 경우에만 다룹니다.
