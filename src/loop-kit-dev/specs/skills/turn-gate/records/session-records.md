...
- `000-plan.md`는 당일 작업의 히스토리, 사용자 요청 목록, flow index, 현재 snapshot, prepared flow sequence 또는 candidate handoff, 완료 flow 요약을 소유하되 bounded date-level index로 유지한다. 세부 형식은 `templates/plan.md`가 소유한다.
...
- `000-self-drive.md`는 sequence-level state를 소유한다: sequence objective, prepared flow sequence, active flow index, allowed autonomous actions, prohibited autonomous actions, approval-sensitive checkpoints, endpoint, blocker return conditions, progress note, progress ledger.
- `active_flow_index`는 0-based machine field이며, `000-self-drive.md`에는 사람이 읽을 수 있는 current flow label도 함께 남긴다. numeric index와 prepared sequence label이 충돌하거나 모호하면 flow name/file 또는 slug를 기준으로 reconcile하고, 해소할 수 없으면 다음 autonomous flow로 진행하지 않는다.
...
- `000-plan.md`의 현재 계획은 action checklist가 아니라 sibling `flow` decision에서 파생된 selected flow sequence 또는 candidate handoff여야 한다.
- session record는 sibling `flow`가 산출한 flow label, flow type, scope, completion criteria, verification expectation, handoff condition을 기록한다. 이 필드들의 판단 기준은 `flow`가 소유한다.
- 후보는 실행 승인이 있거나 next-flow에서 선택되기 전까지 active/completed flow로 세지 않는다.
...
- `Planned Flow Sequence`에는 현재 선택됐거나 미래에 실행할 flow만 둔다. 완료된 flow는 `Flow Index`와 `Completed Flow Summaries`의 compact entry로만 남긴다.
...
- 운영 flow가 판단, 설계, 범위 확인에서 끝나는 경우 최소 기록 항목의 selected flow sequence는 sibling `flow` decision에서 나온 candidate handoff로 대체할 수 있다.
...
- 사용자 메시지 해석과 sibling `flow` decision이 기록되고, 실행용 active flows와 섞이지 않았는가?
- 판단, 설계, 범위 확인용 운영 flow 기록이 실제 실행 flow record처럼 보이지 않도록 후속 후보와 handoff 조건을 구분했는가?
- flow sequence가 sibling `flow` skill의 boundary 규칙과 `core/flow-boundaries.md`의 적용 규칙에 맞게 기록됐는가?
