...
- Stop hook의 script 위치, manifest field, release inclusion, block/quiet-exit 조건, JSON output shape를 runtime guidance에 반복하지 않습니다. 필요한 경우 plugin-level `specs/hooks/stop-hook.md`가 상세 소유자라고만 짧게 참조합니다.
...
- Skill/runtime guidance가 Stop hook의 block/quiet-exit 조건을 상세 반복하지 않고 plugin-level hook spec으로 위임하는가?
