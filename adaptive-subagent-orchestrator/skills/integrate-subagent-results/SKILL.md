...
If new independent work or a `PARALLEL_READ` result creates write scope, return to `$adaptive-subagent-orchestrator:dispatch-subagents` with explicit scope even when the prior manifest kept writes main-owned. The dispatcher may return `DIRECT`. Never turn read results into write authority implicitly.
