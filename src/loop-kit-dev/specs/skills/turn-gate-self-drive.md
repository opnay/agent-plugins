...
- planned flow list의 마지막 flow는 commit-readiness gate를 실행하고 commit-ready 보고를 남겨야 한다. 이 보고는 commit execution approval이 아니며, 실제 commit/push/PR/publish는 별도 user-gated handoff로만 진행한다.
...
- planned flow list execution과 final commit-readiness reporting 규칙은 self-drive overlay의 책임으로 유지하되, 초기 flow-list preparation, 예상 위험 작업 질문, approval boundary 정의는 같은 플러그인의 `turn-gate` skill 책임으로 유지한다.
