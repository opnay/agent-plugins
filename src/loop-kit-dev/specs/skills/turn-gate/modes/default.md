...
사용자가 명시적으로 self-drive 가능한 prepared planned flow sequence를 승인하거나 준비하지 않은 모든 turn-gated flow는 이 기본 상태에서 진행합니다.
...
- external action, destructive action, commit, push, PR, publish, release, version bump는 기본 상태에서도 user-gated handoff로 남긴다.

## 검토 질문

- self-drive 조건이 명확하지 않은데 self-drive로 기록하지 않았는가?
