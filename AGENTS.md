...
- 웹 조사 후 위키에 반영할 내용이 있는지 항상 판단합니다. 검색어나 방문 페이지별로 기록하지 않고 해결한 질문과 결과를 기준으로 묶습니다.
- 기록·갱신·생략 기준은 `$agent-wiki:writer`를 따릅니다. 기준을 충족하면 허용된 위키에 반영하며, 기존 기록 권한과 읽기 전용·기록 금지 지시는 유지합니다.
