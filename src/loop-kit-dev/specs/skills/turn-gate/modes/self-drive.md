...
`self-drive`는 preparation에서 기록된 prepared flow sequence에 autonomous continuation을 적용하는 overlay입니다.
...
- `self-drive` overlay는 prepared flow sequence, scope, non-goal, acceptance signal, approval boundary, verification expectation이 충분히 기록된 뒤에만 적용한다.
- self-drive가 적용되기 전에 session record에는 self-drive active 여부, sidecar pointer, sequence objective, prepared flow sequence, active flow index, current flow label, allowed autonomous actions, prohibited autonomous actions, approval-sensitive checkpoints, endpoint, blocker return conditions, progress note가 드러나야 한다.
...
- self-drive에 필요한 prepared flow sequence와 approval boundary가 기록돼 있는가?
