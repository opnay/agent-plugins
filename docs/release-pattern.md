...
- dev와 release의 이름은 build 변환에서 명확히 구분되어야 합니다.
...
- The dev plugin is a source surface; direct installation is not required.
...
7. When the work is ready for public release, merge `next` to `main` with the already chosen dev version.

## Development Source on `next`

The `next` branch owns the in-progress source under `src/<plugin-name>-dev`.
The `-dev` name distinguishes source from generated release output.
Direct installation and marketplace exposure are optional and are not repository quality requirements.

Expected development flow:

1. Edit and inspect `src/<plugin-name>-dev`.
2. Validate its specs, manifest, skills, and fixtures.
3. Build the root release surface.
4. Validate the generated release surface.
...
로컬 개발은 `src/` 아래의 committed dev 플러그인을 원본으로 사용합니다.
...
dev plugin manifest의 `name`은 source/release 변환을 식별하기 위해 `<plugin-name>-dev`를 사용합니다.
dev 원본의 직접 설치나 공개 플러그인과의 동시 설치는 전제하지 않습니다.

예:

- 개발 원본: `src/example-kit-dev`
- release 산출물: `example-kit`
...
- dev 원본의 직접 설치 가능성을 개발 완료 조건으로 삼지 않습니다.
