...
- 사용자 메시지 기반 preparation이 완료되어 planned flow list, 각 flow의 work boundary, non-goals, verification expectation, 예상 위험 작업, approval boundary, user-gated checkpoint가 기록됐고, 그 flow list를 사용자 추가 응답 없이 이어가야 하는 경우
...
- self-drive는 사용자 메시지 기반 preparation을 대신하지 않는다. 먼저 `turn-gate` preparation이 planned flow list 전체에 필요한 intent, scope, non-goal, acceptance signal, verification expectation을 수집하고, 예상되는 위험 작업과 approval boundary를 질문해 승인/비승인 또는 handoff 경계를 계획해야 한다.
- self-drive는 준비된 planned flow list를 순서대로 실행하되, 각 flow 시작 전 flow record의 work boundary, non-goals, verification expectation, 예상 위험 작업, approval boundary, user-gated checkpoint를 다시 확인한다.
- 이 planned flow list는 phase list가 아니라 응집된 변경 단위 list여야 한다. self-drive는 `분석`, `작업`, `검증`, `커밋 준비` 같은 core phase나 internal mode를 별도 flow로 실행하지 않는다.
- flow가 최종 사용자에게 직접 보이는 가치 단위일 필요는 없다. 보이지 않는 UI/UX component scaffolding, logic layer, integration assembly처럼 검토/검증/커밋 가능한 변경 묶음이면 flow가 될 수 있다.
- 준비된 정보로 진행 가능한 flow는 사용자에게 다시 묻지 않고 self-drive packet과 bounded subagent decision으로 이어간다.
- 준비되지 않은 scope gap, 초기 협의 범위 밖의 위험 작업, 새 approval boundary, 또는 계획되지 않은 commit/push/PR/publish 결정이 발견되면 self-drive를 멈추고 같은 플러그인의 `turn-gate` user-gated question routing으로 다시 질문한다.
- planned flow list의 마지막 변경 단위 flow가 끝나면 commit-readiness gate 성격의 보고를 남겨야 한다. 이 보고는 commit execution approval이 아니며, 산출물 변경을 소유하지 않는 한 새 planned flow가 아니다. 실제 commit/push/PR/publish는 별도 user-gated handoff로만 진행한다.
...
- 초기 preparation에서 예상 위험 작업을 질문했고, 이후 flow에서 초기 협의 밖 위험 작업이 나타날 때 다시 질문하도록 했는가?
...
- planned flow list execution과 final commit-readiness reporting handoff 규칙은 self-drive overlay의 책임으로 유지하되, 초기 flow-list preparation, 예상 위험 작업 질문, approval boundary 정의는 같은 플러그인의 `turn-gate` skill 책임으로 유지한다.
