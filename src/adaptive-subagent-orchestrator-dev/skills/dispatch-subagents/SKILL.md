...
Do not load the integrator if the manifest is incomplete. Otherwise tell the main agent to apply `$adaptive-subagent-orchestrator-dev:integrate-subagent-results` with the manifest. Use [examples.md](references/examples.md) only for boundary-case comparison or validation.
