...
- `예약 플로우 재검토`: 이미 계획된 후보, pending question, self-drive 다음 항목이 현재 선택지에 영향을 주는지 확인
...
`000-plan.md` 업데이트는 선택된 다음 flow 입력, pending/answered question 상태, next action을 매번 반영합니다. Active skill 목록은 flow entry 또는 post-reporting continuation boundary의 skill reconfigure가 갱신합니다.
