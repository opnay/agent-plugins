...
- change spec은 변경 목적, 포함 변경, 비목표, 호환성/마이그레이션, 검증 기준을 기록합니다.
- 독자는 change spec만 보고 다음을 빠르게 파악할 수 있어야 합니다.
  - 무엇이 바뀌었는가
  - 왜 release/change 단위로 의미가 있는가
  - 호환성이나 migration 영향이 있는가
  - 어떻게 검증됐는가
- change spec은 사용자와의 질문/답변 히스토리를 보관하지 않습니다.
...
- plugin spec과 skill spec은 `사용자 스펙 의도`를 둡니다.
- plugin spec과 skill spec의 `사용자 스펙 의도`는 파일 맨 위에 둡니다.
- `사용자 스펙 의도`는 정규 스펙 본문으로 옮기기 전의 입력 기록입니다.
- 사용자 요청문을 시간순으로 기록합니다.
- 질문 도구를 사용했다면 질문과 답변도 함께 기록합니다.
- change spec에는 `사용자 스펙 의도` 형식을 적용하지 않습니다.
- change spec에 필요한 사용자 결정은 릴리즈노트에 필요한 변경 근거로만 짧게 남깁니다.
- `사용자 스펙 의도`의 기본 형식은 아래처럼 사용자 메시지 단위로 시간순 기록을 유지합니다.
...
- skill spec을 수정했고 실제 skill 본문의 문구 작성이 필요한 경우, main agent가 직접 문구를 완성하지 않고 subagent에게 수정된 spec을 기준으로 skill 본문 재작성을 지시합니다.
