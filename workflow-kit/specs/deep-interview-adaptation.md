...
- 현재 환경의 시작점인 `workflow-kit-guide`와 자연스럽게 결합되도록 `deep-interview`를 재정의한다.
...
- global 시작점은 `workflow-kit-guide`다.
...
- 시작점: `workflow-kit-guide`
- 질문 도구: `request_user_input` 또는 일반 대화 질문
- handoff: `workflow-kit` 내부 다른 skill 또는 downstream specialist plugin
...
1. 사용자가 작업이나 질문을 던진다.
2. `workflow-kit-guide`가 현재 workflow를 선택한다.
...
### `workflow-kit-guide -> deep-interview`로 가야 하는 경우
...
### `workflow-kit-guide -> structured-thinking`으로 가야 하는 경우
...
1. `workflow-kit-guide`가 proposal / direction evaluation / greenfield setup에서 `deep-interview`를 더 우선 고르게 할 것
