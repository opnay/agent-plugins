...
- 포함:
  ...
  - 읽기 전용 bounded verifier subagent를 통한 clean-context verification과 중복 검증을 피하는 최소 충분 verifier packet 구성
...
- clean-context verifier는 유지하되, 이미 work 중 충분히 실행되고 기록된 command/check를 근거 없이 반복하지 않고 flow 위험도와 evidence 상태에 맞는 최소 충분 packet으로 검증한다.
