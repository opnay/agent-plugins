...
- approval boundary는 명시적으로 기록된 initial agreement 또는 user-gated 질문으로만 충족한다. subagent, inferred intent, prior nearby wording으로 approval을 대신하지 않는다.
- approval이 필요한데 initial agreement가 부족하면 work로 진행하지 않고 active question-routing으로 target/risk/scope와 종료 지점을 제시한다.
...
- readiness request는 readiness evidence만 제공한다.
- commit approval은 staged/final status, intended diff, unrelated changes exclusion, commit message scope, 종료 지점을 확인한 뒤 별도 commit step 또는 handoff로 넘긴다.
- push, PR, publish 같은 external action은 branch, remote, target, external effect, risk, recovery path, 종료 지점을 확인한 뒤 matching external-action step 또는 handoff로 넘긴다.
...
- external action이 initial agreement 또는 user-gated approval 없이 실행되지 않았는가?
- approval-sensitive execution의 target, effect, risk, recovery, 포함/제외 scope가 기록돼 있는가?
