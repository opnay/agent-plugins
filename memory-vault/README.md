...
사용자가 직접 "기억해"라고 말한 내용뿐 아니라, 에이전트가 작업 중 확인한 실수, 권한 제약, 웹검색 지식, 코드 기반 지식도 재사용 가능하면 저장 후보로 다룹니다.
...
```text
작업 중 확인한 pnpm install 권한 실패를 다음 작업에서 참고할 수 있게 저장해 주세요.
```

## 제공 기능

- `memory-vault`: 개인/작업 장기 메모리 vault를 초기화하고, 사용자 선호, 결정, 환경, workflow, 용어, 미해결 질문, 작업 중 얻은 지식, 번호가 붙은 지식 문서를 분류해 관리합니다.
