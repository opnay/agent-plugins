---
name: rpg-kit-guide
description: Entrypoint skill for `rpg-kit`. Use it to decide whether a task is about role-based subagent orchestration and route to `subagent-role` when role packets, spawn boundaries, answer contracts, or learning notes are needed.
---
...
Use this skill as the default entrypoint for `rpg-kit`.
...
- Do not use `rpg-kit` for ordinary implementation, review, or planning when subagent role orchestration is not the main issue.
...
- Do not hide a broader workflow-selection problem inside `rpg-kit`.
