...
- `turn-gate`는 대화 응답 자체를 제어하는 1급 운영 규칙이어야 한다.
- 결과 보고는 terminal summary가 아니라 다음 플로우 선택으로 이어져야 한다.
- 현재 phase의 작업 성격에 맞는 internal mode를 `turn-gate` 안에서 고르고, mode별 실행 계약은 local `references/`를 읽어 적용하길 원한다.
- 질문, scope lock, next-flow 선택, 승인 경계는 user-gated question routing과 질문 도구로 처리하길 원한다.
- 여러 flow를 거치는 작업은 `.agents/sessions/{YYYYMMDD}/000-plan.md`와 `001+` flow record로 나뉘어 기록되길 원한다.
- 검증 단계는 main agent가 같은 context에서 직접 수행하지 않고, clean-context subagent가 수행해야 한다.
...
`turn-gate`의 세부 계약은 이 index가 직접 모두 소유하지 않고, 아래 sub-spec으로 분리합니다.
