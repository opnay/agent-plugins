...
저장소 루트에서 `go test ./toolkit/skills/git/scripts/git-codex.go ./toolkit/skills/git/scripts/git-codex_test.go`를 실행합니다. 테스트는 임시 Git 저장소를 사용하며 실제 사용자 저장소·원격·설치를 변경하지 않습니다. 플랫폼별 실행 파일은 같은 Go 원본을 `CGO_ENABLED=0`, `GOOS=darwin|linux`, `GOARCH=amd64|arm64`로 빌드합니다.
