...
긴 task list, successful verification, subagent availability, 이전 대화의 의욕적 표현만으로 추론하지 않습니다.
...
`infinite`는 `loop_count`, `current_loop_label`, next bounded iteration을 가집니다. 큰 todo list로 infinity를 표현하지 않습니다.
...
endpoint가 next inventory cycle이면 새 bounded finite cycle로 sidecar를 refresh하고 first flow identity, planned count, acceptance, verification, `next_action`, ledger를 다시 잠급니다.
...
사용자가 "또 8개"처럼 추가 bounded batch를 요청하면 현재 infinite state 안의 endpoint/order update로 처리합니다.
기존 `loop_count`는 ledger history로 보존하고, 새 batch objective, stop condition, next bounded target을 sidecar에 다시 잠급니다.
