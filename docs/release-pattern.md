...
- 일반 개발 작업은 `src/<plugin-name>-dev`만 수정합니다.
- 루트의 release surface는 명시적인 release 생성 또는 release 승격 작업에서만 갱신합니다.
...
기능 수정, skill 문구 수정, spec 수정, 평가 설계 수정은 기본적으로 dev-only 작업입니다.
이때 수정 대상은 `src/<plugin-name>-dev`로 제한합니다.
루트의 `<plugin-name>/` release surface는 함께 고치지 않습니다.

release surface를 갱신하려면 사용자 요청이나 작업 계획에 다음 의도가 명시되어야 합니다.
...
이 의도가 없으면 release generation command를 실행하지 않고, root release diff가 생겼다면 커밋 전에 제외합니다.
...
4. release 승격이 명시적으로 요청된 경우에만 release generation contract에 따라 release surface를 생성하거나 갱신합니다.
...
- dev-only 작업에서는 루트 release surface를 수정하지 않습니다.
