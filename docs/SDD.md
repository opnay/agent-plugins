...
- spec은 구현 세부보다 의도, 경계, 책임 배치를 먼저 고정합니다.
- 파일 종류나 구현 관성을 적더라도 핵심 판단 기준은 change pressure, ownership 이유를 먼저 설명합니다.
...
plugin spec은 bundle 목적, 경계, 사용 표면, skill composition을 소유합니다.
...
child spec은 lifecycle, verification처럼 자기 세부 처리 계약만 소유합니다.
...
이 입력 기록은 사용자가 plugin, skill, runtime behavior, 또는 지속 설계가 어떻게 되길 원하는지를 남기는 표면입니다.
파일을 만들기, 옮기기, 분리하기, 이름 바꾸기처럼 이번 작업에서 수행한 authoring operation 자체를 그대로 보관하는 표면이 아닙니다.
authoring operation이나 migration 이력은 change spec 또는 session record에 두고, 그 요청이 지속 행동 계약을 드러내는 경우에만 원하는 동작이나 책임 기준으로 번역해 `사용자 스펙 의도`에 남깁니다.
...
단, 사용자 메시지가 순수하게 spec 파일 편집 작업을 지시한 것이라면 그 문장을 `사용자 스펙 의도`에 그대로 추가하지 않습니다.
그 경우에는 change spec에 변경 근거를 남기고, `사용자 스펙 의도`에는 사용자가 원한 지속 behavior 또는 책임 경계가 실제로 확인된 경우에만 그 의미를 기록합니다.
