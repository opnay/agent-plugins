...
- requirement discovery, review handling, fix loop, broad execution, commit-readiness 같은 flow-local strategy는 sibling `flow` decision으로 적용한다.
- flow-local strategy는 mode가 아니며, flow type, operating state, next-flow authority를 대체하지 않는다.
...
- 현재 flow에 필요한 strategy를 mode와 분리해 설명했는가?
