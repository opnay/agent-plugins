...
description: Entrypoint skill for the `advance-codex` plugin. Use when a task involves strengthening a Codex surface and Codex should first classify whether the primary deliverable is a skill, tool-use guidance layer, plugin bundle, session management surface, or subagent workflow or custom agent definition.
---
...
- Choose `session-manager` when the main output is repository-local session continuity through `.sessions/<uuid>/` records rather than a new artifact shape.
...
- When building a new multi-skill plugin, prefer this order:
  ...
  5. subagent guidance if the plugin needs it
