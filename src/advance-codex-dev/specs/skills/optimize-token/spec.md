...
`optimize-token`은 skill 호출 시 `light`, `standard`, `extreme` 중 하나의 압축 강도를 선택해 Codex의 응답, 작업 전 판단 문장, 진행 보고를 짧고 선명하게 만들되 정확성, 안전 경계, 검증 보고, 사용자가 요구한 형식을 약화하지 않는 토큰 최적화 skill입니다.
목표는 토큰 낭비를 줄이는 실용적인 작성 기준을 제공하는 것입니다.
...
- 포함:
  ...
  - `response`와 `thinking` 표면 모두에 같은 `light`, `standard`, `extreme` 강도 선택을 적용하기
...
- 사용자가 `$advance-codex:optimize-token light`, `$advance-codex:optimize-token standard`, `$advance-codex:optimize-token extreme`처럼 호출 강도를 지정해 응답과 진행 문장의 밀도를 조절하려는 경우
...
- sub-spec:
  ...
  - `intent-scenarios/response.md`: 요청별 `Before`, `light`, `standard`, `extreme` 응답 비교 예시와 판정 기준
- 호출 방식: 사용자가 짧고 선명한 응답, 토큰 절약, 불필요한 말 제거, 응답 압축을 요청할 때 호출한다.

## 핵심 처리 계약

- `SKILL.md`는 언제 이 skill을 쓰는지와 어떤 runtime reference를 읽을지 안내합니다.
- `SKILL.md`는 `light`, `standard`, `extreme`을 skill 호출 전체에 적용되는 압축 강도로 설명합니다.
- 자세한 응답 작성 규칙은 runtime `references/response.md`가 소유합니다.
- 작업 전 간결한 판단 문장과 사용자에게 보이는 진행 보고를 간결하게 만드는 규칙은 runtime `references/thinking.md`가 소유합니다. 이 규칙은 내부 추론 공개를 요구하지 않으며, skill 호출 강도를 해당 표면에 적용합니다.
- spec의 `response.md`는 runtime response reference의 지속 계약을 소유합니다.
- `intent-scenarios/thinking.md`는 runtime thinking reference의 `light`, `standard`, `extreme` 호출 모드별 출력 차이를 같은 상황과 원문 안에서 비교하는 의도 시나리오를 소유합니다.
- `intent-scenarios/response.md`는 runtime response reference의 `light`, `standard`, `extreme` 호출 모드별 출력 차이를 같은 요청과 원문 안에서 비교하는 의도 시나리오를 소유합니다.
...
- 선택된 압축 강도가 응답과 thinking/progress wording 모두에 일관되게 적용되는가?
- 단순한 "짧게" 요청을 과도하게 `extreme`으로 해석하지 않았는가?
...
- 독립 실행이 필요한 이유: `optimize-token`은 토큰 최적화만 소유하는 작은 skill이므로 단독으로 읽혀야 합니다.

## 확장 원칙

- 더 많은 응답 유형이 필요하면 spec `response.md`, `intent-scenarios/response.md`, runtime `references/response.md`에 추가합니다.
- 작업 전 판단 최적화 예시가 늘어나면 `intent-scenarios/thinking.md`에 먼저 추가하고, `light`, `standard`, `extreme` 호출 강도에 공통으로 필요한 지속 규칙만 runtime `references/thinking.md`로 승격합니다.
