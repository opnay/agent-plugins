---
name: advance-codex-dev-guide
description: Entrypoint skill for the `advance-codex-dev` plugin. Use when a task is about improving how Codex is used and you should first classify whether the primary deliverable is a skill, empirical prompt-evaluation workflow, tool-use guidance layer, plugin bundle, session folder convention, commit workflow, subagent handoff gate, or subagent/custom agent definition.
---

# Advance Codex Dev Guide

## Overview

Use this skill as the default entrypoint for the `advance-codex-dev` plugin.
...
- When `skill-creator` applies, pair the canonical system `skill-creator` with the local `advance-codex-dev:skill-creator` extension.
...
- When `plugin-creator` applies, pair the canonical system `plugin-creator` with the local `advance-codex-dev:plugin-creator` extension.
