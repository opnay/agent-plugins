...
Use this plugin skill as a self-drive overlay that runs beside `turn-gate` in `workflow-kit`.
...
It is not a surface owned by `turn-gate`; it is a `workflow-kit` plugin skill that depends on `turn-gate` for base loop continuity.
