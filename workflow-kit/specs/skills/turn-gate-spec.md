...
- 사용자 메시지: "현재 프론트엔드 프레임워크 뭐야?"
- 분석: 현재 레포지토리의 프론트엔드 프레임워크를 찾아 알려달라는 요청입니다.
- 계획:
  1. `package.json`을 확인합니다. fallback: `deno.json`, `deno.jsonc`, `deno.mod`를 확인합니다.
  2. 결과를 정리합니다.
- 작업: `package.json`을 읽고 프레임워크 패키지를 탐색합니다.
- 결과 보고: 현재 프레임워크는 `Next.js v16`이 설치되어 있습니다.
- 사용자 응답:
  - 다음 플로우는 어떤걸 진행하시나요?
  - 1. `Next.js` 패키지 업데이트
  - 2. `Next.js` 프레임워크 단위 점검
  - 3. `Next.js` 프레임워크 설명
