...
이 protocol은 mode가 아니며, commit execution approval과 분리된 readiness reporting 계약입니다.
...
- 사용자가 commit 준비, readiness, 최종 점검을 요청했지만 commit 실행을 승인하지 않았다.

## 부적합 기준

- broad implementation 자체가 남아 있다.
- intended change unit이 아직 불명확하다.
- commit execution, staging, push, PR, publish, release, version bump가 목적이다.
...
- readiness request를 commit approval로 해석하지 않는다.
- stage, commit, push, PR, publish, release, version bump는 별도 user-gated handoff로 남긴다.

## Handoff

- readiness가 `ready`면 commit execution 여부를 next-flow question으로 연다.
- readiness가 부족하면 earliest safe phase로 돌아가 보완 work 또는 question-routing을 연다.
- self-driven planned flow sequence에서는 마지막 change-unit flow 이후 reporting 또는 handoff 상태로 다루며, 산출물 변경이 없으면 새 planned flow로 만들지 않는다.

## 검토 질문

- intended change unit이 충분히 잠겨 있는가?
- readiness 판단에 필요한 verification evidence가 있는가?
- commit readiness와 commit approval을 구분했는가?
