...
`rpg-kit`은 역할이 할당된 subagent를 어떻게 설계하고, 언제 spawn하며, 그 결과를 어떻게 읽어 다음 orchestration 규칙을 배울지 다루는 plugin입니다.
...
- 대표 스펙: `rpg-kit/specs/plugin.md`
- skill 상세 스펙 위치: `rpg-kit/specs/skills/*.md`
- 평가 설계: `rpg-kit/specs/subagent-role-packet-evaluation.md`

## 내장 skill 체계

- `rpg-kit-guide`: 현재 작업이 `rpg-kit`의 역할 기반 subagent orchestration 범위인지 판단하고 적절한 skill로 라우팅한다.
  - spec: `rpg-kit/specs/skills/rpg-kit-guide.md`
- `subagent-role`: 역할이 할당된 subagent를 위한 packet, spawn rule, answer contract, learning note를 정의한다.
  - spec: `rpg-kit/specs/skills/subagent-role.md`
...
- 초기 버전은 `rpg-kit-guide`와 `subagent-role` 두 skill만 포함한다.
