...
- 모듈이나 처리 단계 경계를 새로 만들면 각 경계의 입력, 출력, 실패, 부수 효과와 대표 end-to-end data flow를 함께 확인합니다.
- 도메인·조정·기반 책임을 나누거나 옮기면 정책 결정의 소유자, 경계 변환, 의존 방향을 확인합니다. 도메인이 외부 표현을 알거나, 조정이 제품 정책을 결정하거나, 기반이 제품·화면 타입에 의존하는 경로가 없어야 합니다.
- 재사용 경계를 만들면 공유 계약의 현재 소비자를 확인하고 focused contract 검증과 대표 소비자 경로를 함께 실행합니다. 단일 소유자 규칙을 추출했다면 추가 비용보다 유지보수·검증 이익이 큰 근거도 확인합니다.
