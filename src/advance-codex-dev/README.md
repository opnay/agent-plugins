...
재사용 가능한 skill, installable plugin bundle, change finalization, reusable instruction evaluation, Notion-backed agent work memory, agent token optimization 같은 Codex 활용 체계를 설계하고 정리하는 작업을 위한 문서화와 가이드를 제공합니다.
...
`notion-memory`는 Notion DB를 Codex 작업 메모리로 설정하거나 작업 히스토리, 결정, 후속 작업, 검증, 재사용 가능한 작업 지식을 기록해야 할 때 사용합니다. Notion I/O는 `ntn` CLI를 1순위로 사용하며, 일반 Notion workspace 자동화에는 사용하지 않습니다.
