...
- `loop-kit-dev`이 현재 작업의 시작점이 맞는지 빠르게 판단하고 싶다.
...
- 사용자 개입 없이 계속 진행해야 하면 `turn-gate-self-drive`로 subagent에게 질문하게 하고 싶다.

---

# loop-kit-dev-guide 스킬 스펙

## 목적

`loop-kit-dev-guide`는 현재 요청이 `loop-kit-dev`의 narrow loop surface로 시작해야 하는지 판단하고, 맞다면 `turn-gate` 또는 `turn-gate-self-drive`로 진입시키는 entrypoint skill입니다.

## 경계

- 포함:
  - `loop-kit-dev` 적합성 판단
  - `turn-gate` 또는 `turn-gate-self-drive` 시작 여부 판단
...
- 대표 표면: `loop-kit-dev/skills/loop-kit-dev-guide/SKILL.md`
- 관련 상위 라우팅: 없음
...
- 사용자 개입 없는 자동 진행이 governing contract이면 `turn-gate-self-drive`를 선택한다.
- 사용자에게 `ralph-loop`, `review-loop` 같은 direct loop entrypoint를 제안하지 않는다.
- turn continuity가 핵심 계약이 아니면 `loop-kit-dev`보다 `workflow-kit` 또는 다른 plugin이 더 적합할 수 있음을 명시한다.
...
- bounded decision을 subagent question packet으로 라우팅해 자동 진행해야 하는 self-driving task라면 `turn-gate-self-drive`로 보낸다.
- broad workflow selection 자체가 먼저 필요하면 `workflow-kit` 쪽 시작점을 우선 검토한다.

## 검토 질문

- 지금 진짜로 loop continuity가 top-level contract인가?
- direct loop skill을 사용자 표면으로 열지 않고 `turn-gate` 또는 `turn-gate-self-drive`로 진입시켰는가?
- `loop-kit-dev`보다 broader workflow router가 먼저여야 하는 상황을 놓치지 않았는가?

## 독립성 원칙

- 이 skill이 독립 실행 가능성을 spec으로 강제해야 하는가: 아니오.
- 그렇다면 왜 필요한가 / 아니라면 어떤 sibling context를 허용하는가: 이 skill은 `turn-gate`가 base 실행 표면이고 `turn-gate-self-drive`가 overlay라는 plugin context를 전제로 한다. 다만 `loop-kit-dev`로 들어가야 하는 판단 기준 자체는 이 스펙만 읽어도 이해 가능해야 한다.

## 확장 원칙

- 라우팅 규칙을 늘릴 때는 `loop-kit-dev`의 narrow scope를 흐리지 않는 범위에서만 추가한다.
