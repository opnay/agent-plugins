...
| 20 | Flow-local strategy is active for the flow | Keep the turn-gate phase prefix clean, such as `[verification]`, and record the strategy in flow context if needed. | Print `[verification/review-loop]` or `[verification(/review-loop)]` as if the strategy were a turn-gate protocol. |
...
- Flow-local strategy does not leak into turn-gate phase prefix suffixes.
