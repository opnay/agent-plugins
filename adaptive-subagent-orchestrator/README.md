...
`adaptive-subagent-orchestrator`는 복잡한 software-engineering 요청을 저비용으로 선별하고, 병렬 위임 가치가 확인될 때만 bounded subagent를 생성하고 결과를 검증·통합하는 instruction-only 플러그인입니다.
...
- `$adaptive-subagent-orchestrator:orchestrate-subagents`
- "현재 브랜치를 보안, 정확성, 테스트, 성능 관점에서 독립적으로 검토해줘."
- "로그인, 결제, 인벤토리 실행 경로의 간헐적 실패 원인을 각각 조사해줘."

Focused skill 직접 호출:

- `$adaptive-subagent-orchestrator:dispatch-subagents`: raw request 또는 `RouteDecision`이 필요하며 delegation gate를 우회하지 않습니다.
- `$adaptive-subagent-orchestrator:integrate-subagent-results`: active agent IDs와 완전한 `DispatchManifest`가 필요합니다.
