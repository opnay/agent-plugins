...
전체 sequence objective, planned flow list, active flow index, allowed/prohibited autonomous actions, approval-sensitive checkpoints, endpoint는 `000-plan.md`가 소유합니다.
...
- self-drive active flow에서 기본 template field를 억지로 채우지 않고, reference guidance에 따라 전체 sequence는 plan이 소유하고 flow record는 flow-local snapshot만 남기는가?
