...
`loop-kit-dev-guide`는 현재 요청이 `loop-kit-dev`의 narrow loop surface로 시작해야 하는지 판단하고, 맞다면 `turn-gate`로 진입시키는 entrypoint skill입니다.
...
- 그렇다면 왜 필요한가 / 아니라면 어떤 sibling context를 허용하는가: 이 skill은 `turn-gate`가 메인 실행 표면이라는 plugin context를 전제로 한다. 다만 `loop-kit-dev`로 들어가야 하는 판단 기준 자체는 이 스펙만 읽어도 이해 가능해야 한다.
