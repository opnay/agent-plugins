...
영리한 일회성 수정이 아니라 재현 가능성, 진단 가능성, 명시적 계약, 안전한 반복 개선을 우선합니다.

## 작업 원칙

- 외부 실서비스 의존성보다 결정론적인 fixture를 우선합니다.
- 실패를 숨기지 말고 디버깅하기 쉽게 만듭니다.
- 변경은 좁고, 추적 가능하며, 검증하기 쉽게 유지합니다.
- 관측 가능성은 하네스의 일부입니다.
- 입력, 출력, 실패 모드는 명시적 계약으로 둡니다.

## 변경 Workflow

1. 바꾸려는 하네스 동작, 계약, 워크플로를 식별합니다.
2. 관련 스크립트, 설정, fixture, 문서, 테스트를 먼저 읽습니다.
3. 목표 문제를 해결하는 가장 작은 완전한 변경을 만듭니다.
4. 위험 수준에 맞는 가장 좁고 의미 있는 검증을 수행합니다.
5. 변경 내용, 검증, 남은 불확실성을 보고합니다.
...
- infra failure, harness failure, assertion failure를 구분합니다.
- 재실행이나 트리아지가 예상되면 ad hoc print보다 structured log 또는 machine-readable output을 선호합니다.
- setup과 teardown을 명시적으로 유지합니다.
- 결정론적으로 만들 수 없는 경계는 문서화하고 영향 범위를 제한합니다.

## 검증 규칙

- 설정만 바뀌면 파싱 또는 로드 동작을 검증하고, 실행 영향이 있으면 대표 경로 하나를 실행합니다.
- fixture 또는 sample data가 바뀌면 해당 fixture 의존 시나리오만 다시 실행합니다.
- 핵심 하네스 로직이 바뀌면 타깃 테스트와 대표 end-to-end 또는 integration 경로 하나를 추가로 실행합니다.
- 로깅 또는 리포팅이 바뀌면 machine-readable output 형태와 운영자 가독성을 함께 검증합니다.

## 실패 처리

- 원인을 모른 채 blind retry로 flaky 동작을 덮지 않습니다.
- 상태 확인, 이벤트, assertion으로 해결할 수 있으면 wall-clock sleep에 의존하지 않습니다.
- 로컬 재현이 안 되면 불확실한 지점을 정확히 기록합니다.
- 외부 시스템이 관련되면 하네스 로직을 바꾸기 전에 비결정적 경계를 먼저 분리합니다.

## 금지 사항

- seed나 분명한 이유 없이 randomness를 도입하지 않습니다.
- 깨진 하네스 동작을 숨기는 silent fallback을 추가하지 않습니다.
- 하네스 수정과 무관한 cleanup을 섞지 않습니다.
- 테스트 통과를 위해 assertion을 약화하지 않습니다.
- 호출부 갱신이나 break 문서화 없이 출력 계약을 바꾸지 않습니다.
