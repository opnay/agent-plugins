...
이 계약은 명시적으로 준비된 finite flow sequence 위에 적용되는 overlay로서 self-drive를 소유합니다. self-drive는 기본 `preparation -> work -> verification -> reporting -> next-flow` loop를 대체하지 않습니다.
