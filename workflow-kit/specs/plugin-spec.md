...
`workflow-kit`은 재사용 가능한 execution workflow를 통해 작업 품질을 높이기 위한 플러그인입니다.
핵심 역할은 작업을 시작하기 전에 올바르게 정의하고, 적절한 실행 모드를 고르고, 안전하게 반복 개선하고, material finding을 scope drift 없이 처리하고, 마지막으로 commit 쪽으로 넘겨도 되는 상태인지 판단하는 것입니다.
...
- 핵심 분기: 지금 필요한 것이 definition, execution, review-loop handling, readiness gate 중 무엇인지 먼저 분류한다
...
- 새 workflow skill은 definition, end-to-end delivery, bounded refinement, review handling, final gating과 분명히 다른 work mode일 때만 추가한다.
...
- 현재 플러그인 표면은 task stage와 execution mode를 중심으로 일관적이다.
- 현재의 주요 리스크는 work-definition guidance와 domain-specific implementation guidance의 경계가 흐려지는 것이다.
