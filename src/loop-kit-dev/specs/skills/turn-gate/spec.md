...
- `next-flow gate`는 매번 `예약 플로우 재검토 -> 다음 플로우 선택 -> 000-plan.md 업데이트 -> flow skill: interview`를 기본 경로로 처리합니다.
- 예약 플로우 재검토는 이미 계획된 후보, pending question, self-drive 다음 항목이 현재 next-flow 선택지에 영향을 주는지 확인하는 과정입니다.
...
- handoff 뒤 예약 플로우 재검토로 선택지를 정리하고, 질문 도구 또는 self-drive로 다음 flow를 선택한 뒤 필요한 기록을 업데이트하고 `flow skill: interview`로 돌아오는가?
- `skill reconfigure` 소유권을 `flow`에 맡기고 turn-gate가 flow 시작 전 skill context를 직접 재구성하지 않는가?
