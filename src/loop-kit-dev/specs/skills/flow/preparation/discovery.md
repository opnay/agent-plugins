...
사용자 intent, scope edge, non-goal, tradeoff, acceptance signal이 flow contract 형성의 병목일 때 필요한 질문 주제. Discovery는 주로 intake에서 쓰지만, framing 또는 preparation에서 미해결 필드가 발견되면 해당 단계가 intake로 되돌리는 신호로도 사용한다.

## 계약

- discovery는 사용자 의도와 경계를 압력 테스트해 flow contract를 만드는 flow-local 전략이다.
