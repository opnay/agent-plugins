...
차단은 plugin 설치만으로 활성화되지 않아야 하며, 현재 main Codex session id가 `000-plan.md` 또는 active flow record의 `turn_gate_session_id`와 일치할 때만 armed된 것으로 봅니다.
session id를 확인할 수 없거나 기록된 id와 다르면 hook은 조용히 통과해야 합니다.
