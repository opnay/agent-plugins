...
graph TD
  ...
  subgraph SKILLRECONFIG[skill reconfigure]
    ...
    RESET[기존 skill context 폐기]
    REREAD[각 skill 본문 source에서 새로 읽기]
    ACCEPT[freshly read bodies만 새 active skill set으로 수용]
...
- `skill reconfigure` 그룹은 `<gate:skill-reconfigure>...</gate:skill-reconfigure>`로 감싸며, 다음 flow 질문을 만들기 전에 세션에서 사용중인 전체 skill 목록을 식별하고, 기존에 읽은 skill context를 폐기하고, 각 skill 본문을 source에서 새로 읽고, 이전 대화의 stale skill context가 아니라 freshly read bodies만 새 active skill set으로 수용합니다.
