...
`advance-codex`는 Codex용 재사용 아티팩트를 설계하거나 수정하기 위한 플러그인입니다.
핵심 역할은 먼저 어떤 형태의 아티팩트를 만들거나 다듬으려는지 분류하고, 그다음 더 명확한 경계, 패키징 규칙, tool-use policy 분리, custom agent 가이드를 붙여 안정적인 형태로 정리하는 것입니다.
...
- 제외:
  - 일반적인 제품 구현 workflow
  - Codex 아티팩트 설계와 무관한 도메인 실행 가이드
  - `advance-codex` 맥락 없이 공용 유틸리티를 쌓는 일

## 진입 표면

- 대표 엔트리포인트: `advance-codex-guide`
- 핵심 분기: 지금 다루는 대상이 skill, tool-use guidance, plugin, subagent 중 무엇인지 먼저 분류한다

## 스킬 구성

- `advance-codex-guide`: 아티팩트 형태를 분류하고 적절한 creator skill로 라우팅한다
...
- 새 skill은 기존 표면과 분명히 다른 아티팩트 유형이나 설계 관심사가 있을 때만 추가한다.
...
- 이 플러그인은 execution workflow보다 Codex capability design에 집중한다.

## 현재 의도 점검

- 현재 플러그인 표면은 일관적이다. 포함된 skill은 모두 제품 실행이 아니라 Codex 아티팩트 작성 또는 수정에 관한 것이다.
