...
- `skill reconfigure`는 기존에 읽은 skill context를 잊고, 현재 세션에서 사용중인 skill 본문을 source에서 새로 읽어야 한다.
- wrapper가 활성화된 경우 wrapper skill은 skill reconfigure의 active skill 목록에 포함되어야 한다.
- `skill reconfigure`를 다음 플로우 선택 뒤로 옮기고, 기존 위치에서는 예약 플로우를 재검토해야 한다.
- `skill reconfigure`를 `flow`로 옮긴다. flow entry와 post-reporting continuation boundary에서 필요한 skill을 읽고, 루프 중 잊힌 skill context를 복구하는 용도로 둔다.
...
graph TD
  ...
  subgraph TG[turn-gate / 메인]
    ...
    subgraph QR[next-flow gate]
      ...
      RESERVED -->|질문 도구| NEXTFLOW
      RESERVED -->|self-drive| NEXTFLOW
      NEXTFLOW --> PLAN
...
- `next-flow gate`는 `flow skill: handoff -> 예약 플로우 재검토 -> 다음 플로우 선택 -> 000-plan.md 업데이트 -> flow skill: interview`를 기본 경로로 둡니다.
