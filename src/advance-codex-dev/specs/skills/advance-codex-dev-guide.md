...
- 포함:
  - skill, empirical evaluation, tool-use guidance, plugin, session folder convention, commit workflow, custom agent, subagent handoff gate 중 주된 artifact 분류
...
- "어떤 creator 또는 gate skill을 먼저 써야 하는가"가 병목인 작업
...
- 먼저 주된 reusable artifact나 runtime gate를 하나로 고정한다.
...
- 출력은 artifact 분류, 선택된 creator 또는 gate skill, 실행 순서, 주요 리스크를 포함해야 한다.

## 검토 질문

- 지금의 main artifact가 skill, plugin, tool policy, session folder convention, commit, custom agent, subagent handoff gate 중 무엇인가?
...
- 새 artifact type이나 runtime gate가 plugin boundary 안에서 독립 concern이 될 때만 라우팅 분기를 추가한다.
