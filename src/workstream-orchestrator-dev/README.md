# Workstream Orchestrator Dev

독립적으로 시작할 수 있는 software-engineering·조사·실행 작업 흐름을 bounded subagent에 위임하고, 메인 에이전트가 근거와 변경을 검증·통합하도록 안내하는 instruction-only 플러그인입니다.

## 목적

`workstream-orchestrator-dev`는 software-engineering-only 작업과 검색, browsing, 자료 회수, 코드·문서 조사, 구현, 변환, 검토, 검증이 섞인 cross-domain 작업을 다룹니다. 복잡성이나 작업량이 아니라 실제 독립성과 병렬 이점으로 위임을 결정합니다.
...
- 별도 모듈·서비스·실행 경로·테스트 묶음을 독립적으로 조사하거나 구현할 때
- 같은 코드에서 보안·정확성·성능·품질 관점을 독립적으로 검토할 때
...
- 독립 workstream이 명백하지 않은 일반 구현·리뷰·테스트·디버깅 요청
- 복잡성, 많은 파일, 긴 실행 시간만 있는 작업

순수 조사 보고서의 source methodology는 `deep-research`의 경계입니다. 이 플러그인은 sibling을 runtime 의존성으로 요구하지 않으며, software-engineering과 mixed workstream을 같은 gate와 lifecycle로 처리합니다.
...
```text
$workstream-orchestrator-dev:orchestrate-workstreams 시장 조사를 수행하고 결과를 바탕으로 독립적인 프로토타입 두 개를 구현해 주세요.
```
...
```text
로그인, 결제, 인벤토리 실행 경로의 간헐적 실패 원인을 각각 조사해 주세요.
```
