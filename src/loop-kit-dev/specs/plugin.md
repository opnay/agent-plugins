...
- self-drive는 별도 skill 표면이 아니라 `turn-gate` 내부 reference로 동작하며, base loop gate 자체는 `turn-gate`를 직접 따른다.
- self-drive reference는 초기 preparation을 대신하지 않고, `turn-gate`가 만든 planned flow list, 예상 위험 작업, approval boundary를 실행 입력으로 사용한다.
