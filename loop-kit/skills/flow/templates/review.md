<!--
File name: 000-review.md
Location: .agents/sessions/{YYYYMMDD}/000-review.md
Use one review file per session date.
-->
...
- `[axis]` `{short retrospective note}`
  - invalid: `{optional invalid example}`
  - correct: `{optional corrected pattern}`
  - follow-up: `{optional owner surface or candidate}`
...
- At each flow end, append one compact review note. This is mandatory even when there is no retrospective finding; record a no-finding result.
